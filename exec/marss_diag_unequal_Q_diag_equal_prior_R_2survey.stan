...
transformed parameters {
  ...
  for(i in 1:n_pos) {
    ...
    yymiss2[row_indx_pos[i], col_indx_pos[i]] = yy2[i];
  }
  // Include missing observations
  if(n_na > 0) {
    for(i in 1:n_na) {
      yymiss[row_indx_na[i], col_indx_na[i]] = ymiss[i];
      yymiss2[row_indx_na[i], col_indx_na[i]] = ymiss[i]; // no missing vals in this example
    }
  }
}
model {
  ...
  to_vector(yymiss2) ~ normal(to_vector(xx), SD_obs);
}
