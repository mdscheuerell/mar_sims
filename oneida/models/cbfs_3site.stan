data {
  ...
  int<lower=0> n_pos2; // number of non-missing observations
  int<lower=0> row_indx_pos2[n_pos2];
  int<lower=0> col_indx_pos2[n_pos2];
  int<lower=0> n_pos3; // number of non-missing observations
  int<lower=0> row_indx_pos3[n_pos3];
  int<lower=0> col_indx_pos3[n_pos3];
  real yy[n_pos];       // data
  real yy2[n_pos2];       // data
  real yy3[n_pos3];       // data
  ...
  real b_mu[7]; // prior on mean of offdiagonal
  real b_sd[7]; // prior on sd offdiagonal
...
transformed parameters {
  ...
  for(i in 1:n_pos) {
  xvec[i] = xx[row_indx_pos[i], col_indx_pos[i]];
  }
    for(i in 1:n_pos2) {
  xvec2[i] = xx[row_indx_pos2[i], col_indx_pos2[i]];
  }
    for(i in 1:n_pos3) {
  xvec3[i] = xx[row_indx_pos3[i], col_indx_pos3[i]];
  }
}
model {
  // PRIORS
  ...
  // B off-diagonals
  for(i in 1:7) Boffd[i] ~ normal(b_mu[i],b_sd[i]);
  // LIKELIHOOD
  //for(t in 2:n_year) {
  //  xx[,t] ~ normal(Bmat * xx[,t-1], SD_proc);
    // col(xx,t) ~ multi_normal(Bmat * col(xx,t-1), QQ);
  //}
  yy ~ normal(xvec, SD_obs);
  yy2 ~ normal(xvec2, SD_obs);
  yy3 ~ normal(xvec3, SD_obs);
